...
bool Max72xxPanel::getPixel(int16_t xx, int16_t yy) {
	int16_t x = xx;
	byte y = yy;
	byte tmp;

	if ( rotation ) {
		if ( rotation >= 2 ) { x = _width - 1 - x; }
		if ( rotation == 1 || rotation == 2 ) { y = _height - 1 - y; }
		if ( rotation & 1 ) { tmp = x; x = y; y = tmp; }
	}
